...
for name in tqdm(names_list, position = 0, leave = False):
  ...
  response = client.chat.completions.create(
    ...
    messages = [
      {"role": "system", "content": "Complete the following sentence with a single word or expression. Only return the word or expression. Respond with a name of a video game."},
      {"role": "user", "content": "An American person named {person} is playing a video game. {person} is playing: ".format(person = name['name'])}
    ],
    n = num_iter, 
    max_tokens = 20
  )
...
if save == True:
  ...
  final_df.to_csv('videogames.csv', index = False)
